...
object FindTop5SpeedMonitorInfos {
  def main(args: Array[String]): Unit = {
    ...
    val monitorInfos = sc.textFile("T:/code/spark_scala/data/monitor_flow_action")
    ...
    monitorInfos.map(line => {
      ...
      .map(tp => {
        ...
        var fourthCount = 0
        while (iter.hasNext) {
          val currenSpeed = iter.next().toInt
          if (currenSpeed <= 60) {
            firstCount += 1
          } else if (currenSpeed > 60 && currenSpeed <= 90) {
            secondCount += 1
          } else if (currenSpeed > 90 && currenSpeed < 120) {
            thirdCount += 1
          } else {
            fourthCount += 1
          }
